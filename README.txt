...
Instead, SimpleBASIC omits line numbers and uses labels. It must be the first item on a line. We can then reference that line from anywhere else in our program. Translating the above example:
...
Flow Control
IF expression THEN label. If expression is true, GOTO label
